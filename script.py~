...
try:
    query = "create table mesh_x_"+str(xmesh)+"_y_"+str(ymesh)+"(gid BIGSERIAL PRIMARY KEY); SELECT AddGeometryColumn ('mesh_x_"+str(xmesh)+"_y_"+str(ymesh)+"','geom',4326,'POLYGON',2);"
    ...
    while y1<ymax:
        while x1<xmax:
            x2 = x1 + xmesh
            y2 = y1 + ymesh
            query = "insert into mesh_x_"+str(xmesh)+"_y_"+str(ymesh)+" (geom) select st_transform(st_setsrid(st_geomfromtext('polygon(("+str(x1)+" "+str(y1)+", "+str(x2)+" "+str(y1)+", "+str(x2)+" "+str(y2)+", "+str(x1)+" "+str(y2)+", "+str(x1)+" "+str(y1)+"))'), 3857), 4326)"
            cursor1.execute(query)
            connection.commit()
            x1 = x2
        ...
        row += 1
        column = 1
    print "Completed. Mesh table mesh_x_"+str(xmesh)+"_y_"+str(ymesh)+" has been generated inside database with %s rows and %s columns" % (rows, columns)
except:
    print "Oops! Some error has occurred. May be there is already a table named mesh_x_"+str(xmesh)+"_y_"+str(ymesh)+" inside database" 
